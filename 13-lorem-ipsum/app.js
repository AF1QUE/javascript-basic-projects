...
const content = document.querySelector(".lorem-text");

form.addEventListener("submit", (e) => {
  e.preventDefault();

  let rand = Math.floor(Math.random() * text.length);
  let numsOfPara = parseInt(amount.value);

  if (numsOfPara <= 0 || isNaN(numsOfPara) || numsOfPara > 9) {
    content.innerHTML = `<p>${text[rand]}<p/>`;
    console.log(numsOfPara);
    console.log("inside logic");
  } else {
    const tempText = text.slice(0, numsOfPara);
    console.log(tempText);

    const result = tempText
      .map((text) => {
        return `<p>${text}</p>`;
      })
      .join("");
